...
namespace Core
{
	...
	void RLTexture2D::setFilterMode(RLTextureFilterMode filterMode)
	{
		switch (filterMode)
		{
		case RLTextureFilterMode_Point:
			...
			rlCheckError();
			break;
		case RLTextureFilterMode_Bilinear:
	...
	int32 RLTexture2D::GetDataSizePerPixel() const
	{
		int32 multiplier = 1;

		switch (m_dataType)
		{
		case RLDataType_Float:
			multiplier = sizeof(float);
			break;
		case RLDataType_UnsignedByte:
		...
		{
		case RLPixelFormat_RGB:
			dataSizePerPixel = multiplier * 3;
			break;
		case RLPixelFormat_RGBA:
			dataSizePerPixel = multiplier * 4;
			break;
		case RLPixelFormat_Luminance:
	...
	RLTextureWrapMode RLTexture2D::GetWrapMode() const
	{
		return m_wrapMode;
	}
	...
	RLTextureFilterMode RLTexture2D::GetFilterMode() const
	{
		return m_filterMode;
	}
